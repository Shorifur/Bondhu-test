...
import { PointsIcon, FireIcon, CheckCircleIcon, CrownIcon } from '@/components/ui/CulturalIcons';

const POINTS_ACTIVITIES = [
  { icon: '✍️', action: 'পোস্ট তৈরি', actionEn: 'Created a post', points: 10, time: '22 min ago' },
  { icon: '❤️', action: 'লাইক পেয়েছেন', actionEn: 'Received a like', points: 2, time: '35 min ago' },
  { icon: '👥', action: 'নতুন অনুসারী', actionEn: 'New follower', points: 5, time: '1 hr ago' },
  { icon: '📱', action: 'দৈনিক লগইন', actionEn: 'Daily login', points: 5, time: '3 hrs ago' },
  { icon: '💬', action: 'মন্তব্য করেছেন', actionEn: 'Commented', points: 3, time: '5 hrs ago' },
  { icon: '🔗', action: 'পোস্ট শেয়ার', actionEn: 'Shared a post', points: 5, time: 'Yesterday' },
  { icon: '✍️', action: 'প্রথম পোস্ট', actionEn: 'First post ever', points: 20, time: '2 days ago' },
  { icon: '🔥', action: '৭ দিনের স্ট্রীক', actionEn: '7-day streak', points: 50, time: '3 days ago' },
];
...
export default function PointsPage() {
  ...
  return (
    <div className="min-h-screen pb-20" style={{ backgroundColor: '#F8F7FF' }}>
      ...
      {/* Points Hero */}
      <div className="mx-4 mt-4 p-5 rounded-3xl text-white relative overflow-hidden"
        style={{ background: 'linear-gradient(135deg, #7C3AED, #5EEAD4)' }}>
        ...
        <div className="relative text-center">
          ...
          <div className="flex items-center justify-center gap-1 mt-3">
            ...
            <span className="text-sm font-semibold text-amber-300">7 day streak</span>
          </div>
          ...
          {nextBadge && (
            <div className="mt-4">
              ...
              <p className="text-[11px] text-white/70 mt-1.5 font-bangla">
                {nextBadge.labelBn} পেতে আর {(nextBadge.min - myPoints).toLocaleString('bn-BD')} পয়েন্ট দরকার
              </p>
            </div>
          )}
        </div>
      ...
      <div className="px-4 pt-4">
        <div className="flex items-center justify-between gap-2 overflow-x-auto scrollbar-hide pb-1">
          {BADGES.map((badge) => {
            ...
            return (
              <div key={badge.label}
                className={`flex flex-col items-center gap-1 p-2 rounded-xl min-w-[60px] ${isCurrent ? 'bg-white shadow-sm' : 'opacity-50'}`}
                style={isCurrent ? { boxShadow: '0 2px 8px rgba(167,139,250,0.1)' } : {}}>
          ...
          {POINTS_ACTIVITIES.map((activity, i) => (
            <div key={i} className={`flex items-center gap-3 px-4 py-3 ${i < POINTS_ACTIVITIES.length - 1 ? 'border-b border-[#F5F3FF]' : ''}`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-[#9B8FC0]">{activity.actionEn} · {activity.time}</p>
              </div>
